...
% Last Modified by GUIDE v2.5 29-Nov-2012 10:51:39
...
if redstatus + greenstatus + bluestatus + gccstatus + gccsmoothstatus == 0;
    ...
    for i=rois;
        ...
        if redstatus == 1;
                % set the label
                ylabel('DN');
                plot(handles.results(:,2,1),handles.results(:,3,i),char(handles.markertype(i,:)),'Color',[1 0 0],'LineWidth',2);
        end
        if greenstatus == 1;
                % set the label
                ylabel('DN');
                plot(handles.results(:,2,1),handles.results(:,4,i),char(handles.markertype(i,:)),'Color',[0 1 0],'LineWidth',2);
        end
        if bluestatus == 1;
                % set the label
                ylabel('DN');
                plot(handles.results(:,2,1),handles.results(:,5,i),char(handles.markertype(i,:)),'Color',[0 0 1],'LineWidth',2);
        end
